...
export const createColumns = (getProtocolLogo: (name: string) => string): ColumnDef<Project>[] => [
  {
      ...
      return (
        <div className="flex items-center max-w-36 md:max-w-48 relative z-10">
          <Avatar className="h-8 w-8 flex-shrink-0">
            <AvatarImage src={getProtocolLogo(baseProtocol as string)} alt={protocol as string} />
          </Avatar>
    ...
    cell: ({ row }) => {
      ...
      if (row.original.children && row.original.children.length > 0) {
        const childReasons = row.original.children
          ...
          .filter(
            (reason: string, index: number, arr: string[]) => arr.indexOf(reason) === index // Remove duplicates
          );
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex w-full justify-center">
          <PizzaRosetteCell values={getRiskDescriptions(risks)} isUnderReview={false} />
        </div>
      );
    },
    meta: {
