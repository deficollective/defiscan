...
const getStageInfo = (stage: Stage) => {
  ...
  if (stage === 0 || stage === "I0")
    return { name: "Stage 0", colorClass: "bg-red-500", color: "#ef4444" };
  if (stage === 1 || stage === "I1")
    return { name: "Stage 1", colorClass: "bg-yellow-500", color: "#eab308" };
  if (stage === 2 || stage === "I2")
    return { name: "Stage 2", colorClass: "bg-green-500", color: "#22c55e" };
  if (stage === "R")
    return { name: "Review", colorClass: "bg-gray-500", color: "#6b7280" };
  if (stage === "O")
    return { name: "Others", colorClass: "bg-red-300", color: "#f87171" };
  if (stage === "V")
    return { name: "Variable", colorClass: "bg-gray-400", color: "#9ca3af" };
  return {
    name: `Stage ${stage}`,
...
export const CombinedStageChart: React.FC<{ className?: string }> = ({
  className,
}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Group by stage
      const stageGroups = allReviews.reduce(
        (acc, review) => {
      ...
      const totalTvl = allReviews.reduce(
        (sum, review) => sum + (review.tvl === "n/a" ? 0 : review.tvl),
        0
      );

      // Create ordered data for Others, Stage 0, Stage 1, Stage 2
      const orderedStages = ["O", 0, 1, 2];
      const stageData: StageData[] = orderedStages
        .map((stage) => {
