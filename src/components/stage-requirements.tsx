...
const StageRequirementSection = ({
  ...
  const numericCurrentStage =
    isInfrastructure && typeof currentStage === 'string'
  ...
  if (isCurrentStageO && stageNumber === 0) {
    ...
    statusIcon = '✅';
    statusText =
      hasRequirements && fixedCount > 0
        ? `: ${fixedCount} issue${fixedCount !== 1 ? 's' : ''} resolved`
        : '';
  } else if (
    typeof numericCurrentStage === 'number' &&
    (numericCurrentStage === stageNumber - 1 || (numericCurrentStage === 0 && stageNumber > 0))
  ) {
            ...
            {normalizedRequirements.map((requirement, index) => {
              ...
              return (
                <li key={index} className="flex items-start leading-6">
                  <span className={`mr-2 flex-shrink-0 ${itemColor}`}>{itemIcon}</span>
...
export function StageRequirements({
  ...
  const defaultOpenStage =
    typeof numericStage === 'number' ? getDefaultOpenStage(numericStage) : [];
