  ...
  useEffect(() => {
    const fetchProtocols = async () => {
      try {
        ...
        const protocolsWithLogos = data.filter((protocol) => {
          ...
          .filter((protocol) => {
            ...
            const hasOnlyOStages =
              ...
              protocol.reviews.every(
                (review) => review.stage === 'O' || review.stage?.toString().startsWith('I')
              );
      ...
      <div className="border-t pt-6">
        <div className="mb-4">
          <p className="text-xs text-center text-muted-foreground">
            DeFiScan reviews DeFi protocols to assess their decentralization progress and
            centralization risks. We score protocols from Stage 0 to Stage 2 to help users make
            informed decisions.
          </p>
        </div>
          ...
          <div className="relative h-8 my-2">
            ...
            >
              {extendedProtocols.map((protocol, index) => {
                ...
                const oStageReview = protocol.reviews.find((review) => review.stage === 'O');
                const firstValidReview =
                  validStageReview ||
                  oStageReview ||
                  protocol.reviews.find((review) => !review.stage?.toString().startsWith('I'));
