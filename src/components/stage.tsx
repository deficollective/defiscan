...
const risks = {
  'Central Custody': 'All or some assets are in custody by a centralized entity.',
  'Missing Docs':
    'All or some protocol components and expected performance is not publicly documented.',
  'Closed-Source': 'All or some smart contracts are not published with available source code.',
...
const requisites: { [K in Stage]: (reasons?: Reason[]) => React.JSX.Element } = {
  ...
  '1': () => (
    <div>
      <p>Risks from critical permissions and dependencies are significantly reduced by:</p>
      <ul className="list-disc">
        <li>
          <i>either</i> revoking critical permissions{' '}
        </li>
        <li>
          <i>or</i> establishing a Security Council to control such permissions
        </li>
        <li>
          <i>or</i> enforcing an exit window of at least 7 days so users can withdraw funds in case
          of an unwanted protocol update.
        </li>
      </ul>
      <p>
        Critical risks from external dependencies are mitigated by the implementation of appropriate
        fallback mechanisms.
      </p>
      <p className="mt-4">
        Furthermore, the underlying chain cannot censor users’ transactions and a backup user
        interface exists guaranteeing access to user funds.
      </p>
    </div>
  ),
  '2': () => (
    <div>
      <p>
        Critical permissions have either been revoked or delegated to an on-chain governance system
        with ample time for users to exit in case of an unwanted protocol update. Risks from
        external dependencies have been further reduced such that users’ funds and unclaimed yield
        remain unaffected by a failure.
      </p>
      <p>
        In addition, different independent user interfaces and a fully decentralized underlying
        chain guarantee access to users’ funds at any time.
      </p>
    </div>
  ),
...
const buildStageInfo = (stage: Stage, reasons?: Reason[]) => requisites[stage](reasons);
...
export function StageBadge({
  ...
  return (
    <HoverCard>
      ...
      <HoverCardContent className="z-50 p-4 rounded-md bg-white" side="top" sideOffset={4}>
...
export function StackedStageBadge({
  ...
  const sortedStages = [...filteredStages].sort(
    (a, b) => (stagePriority[b] || 0) - (stagePriority[a] || 0)
  );
    ...
    <HoverCard>
      <HoverCardTrigger className={className}>
        <div className="relative flex items-center">
          {/* Main badge in front */}
          <Badge stage={highestStage} className="relative z-30" />
