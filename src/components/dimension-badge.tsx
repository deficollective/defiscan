...
const riskDescriptions: Record<Dimension, Record<string, string>> = {
  ...
  Upgradeability: {
    ...
    medium:
      'Possible updates may result in the theft or loss of unclaimed yield or may otherwise materially change the system (but user funds remain unaffected)',
  ...
  ExitWindow: {
    ...
    medium:
      'Control of the permissioned functions is delegated to a Security Council OR an exit window of at least 7 days is enforced',
...
const DimensionBadgeInner = ({
  ...
  const getBgColor = () => {
    switch (sentiment) {
      case 'low':
        return '#4ade80'; // green-400 (between green-300 and green-500)
      case 'medium':
        return '#facc15'; // yellow-400 (between yellow-200 and yellow-500)
      case 'high':
        return '#ea580ce6'; // orange-600 at 90% opacity
      case 'neutral':
      case 'UnderReview':
      default:
