...
const components = {
  ...
  a: ({ className, id, href = '', ...props }: AnchorProps) => {
    ...
    if (isInternal && href.startsWith('#')) {
      return (
        <a
          id={id}
          className={cn('font-medium underline text-primary underline-offset-4', className)}
          href={href}
          onClick={(e) => {
            e.preventDefault();
  ...
  ul: ({ className, ...props }: ComponentsProps) => (
    <ul className={cn('my-6 ml-6 text-sm md:text-base list-disc', className)} {...props} />
  ),
  ol: ({ className, ...props }: ComponentsProps) => (
    <ol className={cn('my-6 ml-6 text-sm md:text-base list-decimal', className)} {...props} />
  ),
  ...
  tr: ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
    <tr className={cn('even:bg-accent break-words m-0 border-t p-0', className)} {...props} />
  ),
